...
const Navbar = () => {
  ...
  const current = useSelector((state) => state.designs.current)
  ...
  const parseGraphicJSON = () => {
    ...
    if (currentDesign) {
      ...
      const filtedDAta = currentScene.layers.filter((item) => item.name === "line")
      for (let i in filtedDAta) {
        editor.objects.bringForward(filtedDAta[i])
      }
